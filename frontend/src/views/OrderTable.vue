...
export default {
  ...
  methods: {
    ...
    fillTable() {
      ...
      const saveButtonRenderer = params => {
        ...
        button.addEventListener('click', () => {
          ...
          axios.post('http://' + new URL(window.location.href).hostname + ':8001/api/jobs/setInd_Table/', formData)
          .then(response => {
              console.log(response.data);
          })
          .catch(error => {
      ...
      fetch('http://' + new URL(window.location.href).hostname + ':8001/api/jobs/getSchedule')
        .then((res) => res.json())
    ...
    runGeneticOptimizer() {
      ...
      this.isLoading = true; // show loading icon
      axios
        .post('http://' + new URL(window.location.href).hostname + ':8001/api/jobs/run_preference_learning_optimizer/')
        .then((response) => {
          console.log(response.data);
          this.isLoading = false;
          this.fillTable();
    ...
    runSJF() {
      ...
      this.isLoading = true; // show loading icon
      axios
        .post('http://' + new URL(window.location.href).hostname + ':8001/api/jobs/run_sjf/')
        .then((response) => {
          console.log(response.data);
          this.isLoading = false;
          this.fillTable();
    ...
    runDeadlineFirst() {
      ...
      this.isLoading = true; // show loading icon
      axios
        .post('http://' + new URL(window.location.href).hostname + ':8001/api/jobs/run_deadline_first/')
        .then((response) => {
          console.log(response.data);
          this.isLoading = false;
          this.fillTable();
    ...
    deleteJobs() {
      ...
      this.isLoading = true; // show loading icon
      axios
        .post('http://' + new URL(window.location.href).hostname + ':8001/api/jobs/deleteJobs/')
        .then((response) => {
          console.log(response.data);
          this.isLoading = false;
          window.alert(response.data.message);
    ...
    stopProcess() {
      axios
        .post('http://' + new URL(window.location.href).hostname + ':8001/api/jobs/stop_PL_optimizer/')
        .then((response) => {
          console.log(response.data);
          this.fillTable();
        })
    ...
    upload() {
      ...
      this.isLoading = true;
      axios.post('http://' + new URL(window.location.href).hostname + ':8001/api/jobs/uploadCSV/', formData)
        .then(response => {
          console.log(response.data);
          this.isLoading = false;
          window.alert(response.data.message);
